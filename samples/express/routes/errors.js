...
const VIEWS = path.join(path.dirname(import.meta.url), 'views');
...
const errorRoutes = [
  ...
  { path: '/not-a-function', template: 'error-not-a-function.html', category: 'not_a_function', desc: 'Calling a non-function value' },
  ...
  { path: '/undefined-value', template: 'error-undefined-value.html', category: 'undefined_value', desc: 'Nested property access returns null/undefined' },
  { path: '/syntax-error', template: 'error-syntax-error.html', category: 'syntax_error', desc: 'Template syntax is invalid' },
  { path: '/undefined-block', template: 'error-undefined-block.html', category: 'undefined_block', desc: 'Block not found in parent template' },
  ...
  { path: '/filesystem-error', template: 'error-filesystem-error.html', category: 'filesystem_error', desc: 'OS filesystem error' },
];

errorRoutes.forEach(({ path: routePath, template, category, desc }) => {
  router.get(routePath, async (req, res) => {
    await renderWithErrorHandler(res, template);
  });
});
