...
import { execute, type ExecuteConfig } from './executor.js';
...
const createValidationError = (validationError: ValidationError, stamps: Record<string, unknown>, config: RenderConfig, templateSource: string | null, context: unknown): never => {
  const err = new Error(validationError.message);
  Object.assign(err, stamps);
  throw wrapWithLog(err, config, templateSource, context);
};

const getDangerousValueStamps = (contextError: ValidationError, config: RenderConfig): Record<string, unknown> => {
...
const prepareSandbox = (config: RenderConfig, context: unknown): Record<string, unknown> => {
  ...
  const sandboxedCtx = createSandboxedContext(context, config.sandbox ?? false, sandboxOptions) as Record<string, unknown>;
...
const buildRenderEnv = (loader: unknown, config: RenderConfig): void => {
  ...
  config.env = createEnv({
    ...
    async getTemplate(name: string, eagerCompile?: boolean, includeChain?: unknown[] | null, ignoreMissing?: boolean) {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      return createTemplate(source.src, this as any, source.path, eagerCompile ?? true, includeChain);
    }
  }) as any;
};
...
const handleContextStrictMode = (context: unknown, config: RenderConfig): { warningsCollector: unknown[]; dangerousValuePaths: string[] } => {
  ...
  if (config.contextStrict === 'error' || config.production === true) {
    ...
    throw wrapWithLog(err as Error, config, null, context);
  }
...
const validateRenderInput = (template: unknown, config: RenderConfig, context: unknown): void => {
  if (typeof template !== 'string') {
    const err = createLog('error', ERROR_DEFINITIONS.TEMPLATE_MUST_BE_STRING!, {}, null, { phase: 'render' });
    throw wrapWithLog(err as Error, config, template as string | null, context);
  }
...
export const render = async (template: string, context: Record<string, unknown> = {}, config: RenderConfig = {}): Promise<string> => {
  ...
  try {
    ...
    throw wrapWithLog(err as Error, config, templateSource, context);
  }
...
export const renderWithEnv = async (templateName: string, env: unknown, context: Record<string, unknown> = {}, config: RenderConfig = {}): Promise<string> => {
  ...
  if (!validation.valid) {
    ...
    throw wrapWithLog(err as Error, fullConfig, null, context);
  }
  ...
  try {
    ...
    throw wrapWithLog(err as Error, fullConfig, (template as { tmplStr?: string })?.tmplStr ?? null, context);
  }
};
