...
export const compileBlock = (ctx: Compiler, node: Node): void => {
  ...
  ctx.emitLine(`let ${id} = await (await context.getBlock("${nameNode.value}", ${lineno}, ${colno}))(env, context, frame, runtime);`);
...
export const compileSuper = (ctx: Compiler, node: Node, frame: Frame): void => {
  ...
  ctx.emitLine(`lineno = ${node.lineno}; colno = ${node.colno};`);
