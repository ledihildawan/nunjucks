...
import { funCall, literal, lookupVal, symbol } from '@nunjucks/nodes';
...
describe('extractVarName', () => {
    ...
    const lookupCases: ReadonlyArray<{ label: string; node: Node; expected: string }> = [
      {
        ...
        node: lookupVal(ZERO_LOC, {
          target: symbol(ZERO_LOC, 'foo'),
          val: literal(ZERO_LOC, 'bar'),
        }),
        expected: 'foo.bar',
      },
      {
      ...
      },
      {
        ...
        node: lookupVal(ZERO_LOC, {
          target: symbol(ZERO_LOC, 'user'),
          val: symbol(ZERO_LOC, 'name'),
        }),
        expected: 'user.name',
      },
      {
  ...
  describe('returns null for non-extractable nodes', () => {
    const nullCases: ReadonlyArray<{ label: string; node: Node }> = [
      ...
      {
        label: 'function call (neither symbol nor lookup)',
        node: funCall(ZERO_LOC, { name: symbol(ZERO_LOC, 'fn'), args: [] }),
      },
      {
        label: 'lookup whose target is a literal (no base symbol to walk)',
        node: lookupVal(ZERO_LOC, { target: literal(ZERO_LOC, 'x'), val: literal(ZERO_LOC, 'y') }),
      },
    ];
