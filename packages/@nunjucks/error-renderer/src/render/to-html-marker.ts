...
const MARKER_CSS = `
.nj-err-block{margin:.5rem 0;border:1px solid light-dark(oklch(90% 0.05 25),oklch(35% 0.05 25));border-left:4px solid light-dark(oklch(60% 0.2 25),oklch(65% 0.2 25));border-radius:.375rem;font-family:system-ui,-apple-system,sans-serif;font-size:.8125rem;color:light-dark(oklch(25% 0.02 285),oklch(90% 0.01 285));color-scheme:light dark;overflow:hidden;box-shadow:0 1px 3px rgba(0,0,0,.05);}
...
.nj-err-icon{color:light-dark(oklch(55% 0.2 25),oklch(70% 0.18 25));display:flex;align-items:center;flex-shrink:0;}
.nj-err-tag{background:light-dark(oklch(95% 0.05 25),oklch(30% 0.08 25));color:light-dark(oklch(45% 0.2 25),oklch(70% 0.18 25));font-size:.625rem;font-weight:700;padding:.125rem .375rem;border-radius:.25rem;letter-spacing:.05em;flex-shrink:0;text-transform:uppercase;}
.nj-err-msg{font-family:ui-monospace,SFMono-Regular,Menlo,Monaco,Consolas,monospace;font-weight:600;color:light-dark(oklch(40% 0.15 25),oklch(72% 0.15 25));white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}
.nj-err-btn{background:light-dark(oklch(100% 0 0),oklch(20% 0.01 285));border:1px solid light-dark(oklch(80% 0.05 25),oklch(40% 0.05 25));color:light-dark(oklch(45% 0.2 25),oklch(70% 0.18 25));padding:.125rem .5rem;border-radius:.25rem;font-size:.6875rem;font-weight:600;cursor:pointer;white-space:nowrap;}
...
.nj-err-loc{display:flex;align-items:center;gap:.375rem;padding:.375rem .75rem;background:light-dark(oklch(97% 0.01 25),oklch(20% 0.01 285));border-top:1px dashed light-dark(oklch(90% 0.01 285),oklch(30% 0.01 285));font-size:.6875rem;}
.nj-err-loc-label{color:light-dark(oklch(65% 0.01 285),oklch(55% 0.01 285));font-weight:500;text-transform:uppercase;font-size:.5625rem;letter-spacing:.05em;flex-shrink:0;}
...
.nj-err-loc-link:hover{color:light-dark(oklch(55% 0.2 25),oklch(70% 0.18 25));text-decoration:underline;}
.nj-err-file-icon{flex-shrink:0;opacity:.7;}
.nj-err-pos{color:light-dark(oklch(55% 0.2 25),oklch(70% 0.18 25));font-weight:600;}
...
const buildLocationHtml = (loc: LocData): string => {
  ...
  const inner = `${FILE_ICON}<span class="nj-err-path">${escapeHtml(loc.displayPath)}${posSpan}</span>`;
  const link = loc.canLink && loc.rawPath
    ? `<a href="${resolveIdeLink('vscode', { path: loc.rawPath, line: 0, col: 0 })}" class="nj-err-loc-link" title="Open in VS Code">${inner}</a>`
    : `<span class="nj-err-loc-link">${inner}</span>`;
  return `<div class="nj-err-loc"><span class="nj-err-loc-label">Source:</span>${link}</div>`;
};
...
<div class="nj-err-block" role="status" aria-live="polite">
  <div class="nj-err-header">
    ...
    <button type="button" class="nj-err-btn" data-nj-err-open="${id}" title="View details">Detail</button>
  </div>
  ${locHtml}
</div>
