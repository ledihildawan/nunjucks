...
export const DEFINITIONS = {
	...
	UNDEFINED_BLOCK: createError('UNDEFINED_BLOCK', 'Undefined block: {name}', ['name']),
	...
	FILESYSTEM_ERROR: createError('FILESYSTEM_ERROR', 'Filesystem error: {msg}', ['msg']),
...
export const ERROR_DEFINITIONS = DEFINITIONS;

export const ERRORS: Record<ErrorCode, ErrorMessageFn> = Object.fromEntries(
	Object.values(DEFINITIONS).map(({ name, message }) => [name, message])
) as Record<ErrorCode, ErrorMessageFn>;

export const PATTERNS: Record<ErrorCode, RegExp> = Object.fromEntries(
	Object.values(DEFINITIONS).map(({ name, pattern }) => [name, pattern])
) as Record<ErrorCode, RegExp>;
