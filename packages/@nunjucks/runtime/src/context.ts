...
const createContextFromState = (state: ContextState): Context => {
  const context: Context = {
    ...
    setParentBlockNames(names: string[] | null): Context {
      // WHY: UNION, not replace — multi-level extends threads ONE context down the
    ...
    addBlock(name: string, block: BlockFn): Context {
      ...
      // WHY: own-blocks-only read — the blocks map is engine-internal (compiler
      // registers names via computed own keys), so an inherited Object.prototype
      // member ('constructor', 'toString', …) must never fold into the chain.
    ...
    getBlock(name: string, lineno: number | null = null, colno: number | null = null): BlockFn {
      ...
      // WHY: own-blocks-only for EVERY name — the blocks map only ever holds own
    ...
    getExported(): Record<string, unknown> {
      return Object.fromEntries(
        state.exported.map((name) => [
          name,
          // WHY: same RCE guard as lookup — exported names resolve as own properties only;
