...
import { createSafeString, copySafeness, markSafe } from '@nunjucks/runtime/safe-string';

describe('createSafeString', () => {
  test('stores string value and length', () => {
    const s = createSafeString('hello') as { val: string; length: number };
...
describe('markSafe', () => {
  test('wraps string in SafeString', () => {
    const result = markSafe('hello') as { val: string };
  ...
  test('function wrapper preserves non-string return', () => {
    const wrapped = markSafe(() => 42) as () => number;
