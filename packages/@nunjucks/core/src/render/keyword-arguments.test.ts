...
describe('keyword arguments rendering', () => {
  describe('global functions with kwargs', () => {
    ...
    test('global function with nested destructuring', async () => {
      const formatUser = ({ user: { firstName = 'Anonymous', lastName = '' } = {} } = {}) =>
        `${firstName} ${lastName}`;
      const result = await renderTemplate(
  ...
  describe('filters with kwargs', () => {
    ...
    test('filter without kwargs uses defaults', async () => {
      const myFilter = (val: unknown, { suffix = '!' }: { suffix?: string } = {}) =>
        `${String(val)}${suffix}`;
    ...
    test('chained filters with kwargs', async () => {
      const addPrefix = (val: unknown, { prefix = '' }: { prefix?: string } = {}) =>
        `${prefix}${String(val)}`;
      const addSuffix = (val: unknown, { suffix = '' }: { suffix?: string } = {}) =>
        `${String(val)}${suffix}`;
      const result = await renderTemplate(
...
`;
  ...
  describe('mixed positional and keyword args', () => {
    test('mixed positional and keyword args', async () => {
      const mixed = (
        a: unknown,
        { b = 'default_b', c = 'default_c' }: { b?: string; c?: string } = {}
      ) => `${String(a)}-${b}-${c}`;
      const result = await renderTemplate(
