import { ERROR_DEFINITIONS, getError } from '@nunjucks/error-catalog';
import type { TemplateError } from '@nunjucks/error-formatter';
import { createLog } from '@nunjucks/error-formatter';
import { err, isErr, ok, type Result } from '@nunjucks/lib';
import { validateConfig, validateRenderContext, validateTemplate } from '@nunjucks/validators';
...
const combineValidationErrors = <T extends { message: string }>(
  errors: readonly [T, ...T[]]
): T => {
  ...
  return {
    ...primary,
    message: `${primary.message} (+${rest.length} more: ${additionalMessages})`,
  };
};
...
const getDangerousValueLocationMeta = async (
  contextError: RenderValidationError,
  config: RenderConfig
  ...
  const pos = await findContextKeyPosition({
    sourceFile: callerLocation.fileName,
    callLine: callerLocation.lineNumber ?? 1,
    dangerousPath: firstDangerousPath,
  });
  if (pos) {
...
export const validateRender = async (
  ...
  if (typeof template !== 'string') {
    const error = createLog('error', {
      def: getError('TEMPLATE_MUST_BE_STRING'),
      params: {},
      subject: null,
  ...
  if (isErr(validation)) {
    ...
    const resolvedLineno: number | null | undefined =
      callerLineno && callerLineno > 1 ? callerLineno - 1 : callerLineno;
    return err(
      await buildValidationError({
  ...
  if (isErr(templateValidation)) {
    ...
    return err(
      await buildValidationError({
        validationError: configError,
        stamps: {
          lineno: configError.lineno,
          colno: configError.colno,
          code: configError.code,
          subject: configError.subject,
        },
        config,
