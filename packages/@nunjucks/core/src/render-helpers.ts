...
import type { RenderConfig, LoaderSource, CompileResult, Environment, SandboxOptions } from './render-types.ts';
...
const resolveTemplateSource = async (template: string, loader: Loader | null, config: RenderConfig): Promise<{ templateSource: string; templatePath: string | null }> => {
...
const buildRenderEnv = (loader: Loader | null, config: RenderConfig): Env | null => {
...
const compileTemplate = (templateSource: string, config: RenderConfig, templateName: string): CompileResult => {
  const code = compileToCode(templateSource, templateName, (config.undefined || 'chainable') as 'chainable' | 'strict' | 'debug', { undefined: config.undefined } as ParseOptions);
  return { code };
};
...
const handleContextStrictMode = async (context: unknown, config: RenderConfig): Promise<{ warningsCollector: unknown[]; dangerousValuePaths: string[]; context: unknown }> => {
  ...
  return { warningsCollector, dangerousValuePaths, context: scrubbedContext };
};
