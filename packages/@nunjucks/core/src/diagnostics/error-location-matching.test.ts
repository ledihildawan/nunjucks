...
describe('extractTemplatePosition', () => {
  ...
  test('locates a template literal inside the caller source', () => {
    ...
    const pos = extractTemplatePosition({
      content,
      template: 'Hello {{ name }}',
      errLineno: 0,
      errColno: 8,
  ...
  test('honours preferredLine to disambiguate multiple occurrences', () => {
    ...
    const first = extractTemplatePosition({
      content,
      template: 'x',
      errLineno: 0,
      errColno: 0,
    ...
    const third = extractTemplatePosition({
      content,
      template: 'x',
      errLineno: 0,
      errColno: 0,
  ...
  test('computes a deterministic position for an inline template', () => {
    ...
    ).toEqual({ line: 1, col: 6 });
  });

  test('BLOCKED_CONTEXT_KEYS: locates {{ subject }} instead of [subject] when template matches the pattern', () => {
    const content =
      "const html = await render('{{ password }}', { password: 'secret123' }, { blockedContextKeys: ['password'] });";
    const pos = extractTemplatePosition({
      content,
      template: '{{ password }}',
      errLineno: null,
...
describe('extractQuotedSubjectPosition', () => {
  ...
  test('finds a quoted property key but ignores bare keyword occurrences', () => {
    ...
    const pos = extractQuotedSubjectPosition({
...
describe('extractBareSubjectPosition', () => {
  ...
  test('locates a bare subject token when the template is not found', () => {
    ...
    const pos = extractBareSubjectPosition({
      content,
      template: 'totally different',
      errLineno: 0,
      errColno: 0,
      subject: 'missingVar',
      preferredLine: null,
