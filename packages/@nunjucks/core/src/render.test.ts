...
import { render, type RenderConfig } from './render.ts';
...
describe('inline template error locations', () => {
  test('scrubs dangerous global references from context in dev mode and emits a warning', async () => {
    ...
    // Pure: render must not mutate the caller's context object.
  ...
  test('throws DANGEROUS_CONTEXT_VALUES when contextStrict is error', async () => {
    ...
    const marker = "DANGEROUS_CONTEXT_LOCATION_MARKER";
    ...
    expect(err.colno).toBe(callerLine.indexOf('user.global') + 1);
  });
  ...
  test('auto caller detection points reserved filter errors at the filter key', async () => {
    const cfg = { dev: true, filters: { 'if': (v: unknown) => v } } as unknown as RenderConfig;
...
describe('dompurify per-render isolation', () => {
  test('dompurify config does not leak across renders', async () => {
    // Render 1: restrictive config — only <b> allowed, so <i> is stripped.
    ...
    // Render 2: NO dompurify option — must use the default config (which keeps <i>),
    // proving render 1's ALLOWED_TAGS did not leak into a shared global.
