...
describe('parse - statements', () => {
  test('parses if statement', () => {
    ...
    expect(getNodeTypeName(ast.children[0])).toBe('if');
  });
  ...
  test('parses block', () => {
    ...
    expect(getNodeTypeName(ast.children[0])).toBe('block');
  });

  test('parses extends', () => {
    const ast = parse('{% extends "base.html" %}');
    expect(getNodeTypeName(ast.children[0])).toBe('extends');
  });

  test('parses include', () => {
    const ast = parse('{% include "partial.html" %}');
    expect(getNodeTypeName(ast.children[0])).toBe('include');
  });
