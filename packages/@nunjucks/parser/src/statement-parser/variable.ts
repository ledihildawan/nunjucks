...
import { COMPOUND_ASSIGNMENT_OPS, isSymbolToken, TOKEN_OPERATOR } from '@nunjucks/lexer';
import { isErr, ok, type Result } from '@nunjucks/lib';
import type { Node } from '@nunjucks/nodes';
import { compoundAssignment, variableAssignment, variableDeclaration } from '@nunjucks/nodes';
import { loc } from '@nunjucks/shared';
import type { ParserContext } from '../cursor.ts';
import { fail, nextToken, peekToken, skipValue } from '../cursor.ts';
...
export const parseVariableDeclaration = (
  ...
  if (!patternNode && (!target || (target.type !== 'symbol' && !target.value))) {
    return fail(parserContext, 'Expected variable name or pattern', {
      lineno: tag.lineno,
      colno: tag.colno,
    });
  }
...
const parseOperator = (parserContext: ParserContext, tag: Token): Result<string, TemplateError> => {
  ...
  if (tok?.type === TOKEN_OPERATOR) {
    ...
    return fail(parserContext, 'Expected =, ||= , &&=, ??=, **=, //=', {
      lineno: tag.lineno,
      colno: tag.colno,
    });
  }
...
export const parseVariableAssignment = (
  parserContext: ParserContext
): Result<Node, TemplateError> => {
  const tagR = peekToken(parserContext);
  if (isErr(tagR)) {
