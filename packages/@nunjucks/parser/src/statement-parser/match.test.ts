...
describe('parseMatch', () => {
  test('parses a match with patterns and bodies', () => {
    const node = parseFirst('{% match x %}{% when 1 %}one{% endmatch %}');
  ...
  test('parses multiple when branches', () => {
    const node = parseFirst('{% match x %}{% when 1 %}one{% when 2 %}two{% endmatch %}');
    expect(isMatch(node)).toBe(true);
    if (!isMatch(node)) {
      return;
    }
  ...
  test('parses when branches with guards', () => {
    const node = parseFirst('{% match x %}{% when 1 if y %}one{% endmatch %}');
    expect(isMatch(node)).toBe(true);
    if (!isMatch(node)) {
      return;
    }
  ...
  test('parses a default case via underscore', () => {
    const node = parseFirst('{% match x %}{% when _ %}anything{% endmatch %}');
    expect(isMatch(node)).toBe(true);
    if (!isMatch(node)) {
      return;
    }
  ...
  test('captures the match expression', () => {
    const node = parseFirst('{% match x.y %}{% when 1 %}one{% endmatch %}');
    expect(isMatch(node)).toBe(true);
    if (!isMatch(node)) {
      return;
    }
