...
const parseWhenBranch = (parserContext: ParserContext, tag: Token): Result<WhenNode, TemplateError> => {
...
const parseOneWhen = (parserContext: ParserContext, tag: Token): Result<WhenIteration, TemplateError> => {
...
export const parseMatch = (parserContext: ParserContext): Result<Node, TemplateError> => {
  ...
  return ok(match(loc(tag), { expr: exprR.value, cases: casesR.value.cases, default: casesR.value.defaultCase }));
};
