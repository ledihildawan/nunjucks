...
The factory owns the loader lifecycle (closure-scoped loader cache per `views` path, isolated across factory instances). Internal `render()` callers (core tests) get an uncached loader created from `views`.

**Compiled-code cache** (`cache.templates`, default on): loader-resolved templates cache the compiled code string in a per-engine bounded LRU (`core/src/template/template-cache.ts`; inline template strings skip it by design). The key carries the template path, name, every compile-affecting config field, AND a content hash of the source — so a rewritten file lands on a fresh key on the next render: stale output is structurally impossible and no watcher is required (the loader read that produces the hash is the freshness check). Only successful compiles are cached. `cache: { templates: false }` restores always-recompile behavior. Measured cost split (Windows, bun 1.3.14): loader source resolution (realpath/stat/readFile) is ~6× the compile cost — the compiled-code cache removes the compile share, and the loader verification pass is the next perf lever (would need mtime-keyed source memoization; the dormant watcher hook remains the invalidation path for skipping the read entirely). Include-path (`env.getTemplate`) compiles are not cached yet — the Template object bakes its env at creation, so caching them is a Template-level design change, not a bolt-on.
