...
* **Audit rounds 1–10 fold-in:** frame/context prototype-escape guard parity (`{{ x := 1 }}{{ constructor }}` no longer leaks intrinsics); SafeStrings are attribute-escaped in attribute contexts (`tojson` in `data-x="..."` can no longer break out); `replace` with an omitted replacement deletes the needle (was silent comma-join corruption); unknown filter kwargs throw catalogued `UNKNOWN_FILTER_KWARG` instead of silently misbinding; kwargs binding fixed for `round`/`sanitize`/`join`/`sum`/`slice` (`round(precision=2)` etc.); block rendering is document-order with nested blocks rendering exactly once and 3-level `extends` resolving ancestor blocks; child non-block output is suppressed under `extends` (Jinja parity); `{% exec %}`/`{% from %}` honor `-%}`; anonymous `{% slot %}` works; ternary without `:` is a parse error; two `{% render %}` blocks can share a slot name; duplicate slot names in one render fail with `DUPLICATE_SLOT`; include-emitted `undefined:'debug'` warnings surface once per page on the public API; streaming `executionTimeout` is enforced for CPU-bound (microtask-only) renders; extension-bearing loader misses raise `FILE_NOT_FOUND` instead of rendering the filename as the page; scrubber preserves `Date`/`Map`/class instances and caps scan depth; classify message-length cap; lexer number/comment large-input loops; `BUILTIN_FILTER_NAMES`/`BUILTIN_TEST_NAMES` SSOT drift pins; `@types/bun` declared (clean checkout typechecks); deterministic parser fuzz corpus; biome format gate in CI.
* **Compiled-code cache** (`cache.templates`, default on): loader-resolved templates reuse their compiled code across renders in a per-engine LRU. Cache keys include a content hash of the source, so edited files are reflected on the next render with no watcher and no restart — `false` restores always-recompile.
* **`sanitize` is opt-in (breaking within the 4.0.0 line):** the DOMPurify filter moved to the `@nunjucks/filters/sanitize` subpath and out of the builtin barrel — `isomorphic-dompurify` (jsdom) no longer loads with the pure filter tier. Register it explicitly: `nunjucks({ filters: { sanitize } })` with `import { sanitize } from '@nunjucks/filters/sanitize'`. The `dompurify` engine config still threads defaults onto a registered `sanitize`.
